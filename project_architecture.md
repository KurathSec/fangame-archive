...
**Field `source` (per game)** — a **list** of `{"type": "df" | "wiki", "id": "<upstream id>"}`, every upstream page that documents the game, **ingest origin first**. Only type+id are stored — the page URL is derived at the edges (`functions/api/_lib/catalog.js`, `src/components.jsx`) so the 30 MB catalog does not carry ~28k redundant URL strings. Locally-submitted games (`SUBMISSION-*`) have no upstream page and stay unsourced. Two producers write it, both through `pipelines/source_links.py` (the single home of the shape, the merge rules and the matching rules):

1. **Ingest origin** — a projection of `database/seq_to_orig_map.json` (the long-standing seq→origin mapping). [`backfill_source_links.py`](#83-supporting-scripts) for the historical mass, inline in the scraper for every new ingest (§8.1). `primary=True` puts it first and replaces any earlier entry of the same type, so a re-mapped game follows seq_map.
2. **IWanna Wiki cross-link** — a game ingested from Delicious Fruit usually *also* has a wiki entry, which seq_map never recorded. The wiki's public catalog API exposes an id per entry, so the two catalogs are matched on title. [`backfill_wiki_links.py`](#83-supporting-scripts) applies the committed `data/wiki_links.json` artifact; the scraper matches newly ingested games inline. ~12.3k games end up with both links (≈12.3k of 15.4k DF-sourced ones).

> **Why the matching is deliberately narrow.** A wrong cross-link points a reader at a *different game's* page. So: the normalized title must be unique **on both sides** (exactly one wiki entry and exactly one catalog game carry it — long idiosyncratic fangame titles make this a strong signal, and the ~330 genuine collisions are dropped rather than guessed); a wiki id is never used twice; and the creator acts only as a **veto**. DF stores slug-ish creator names (`lee_ho-seong`) where the wiki stores display names (`Lee Ho-Seong`), so comparison strips every separator and splits the wiki's collaborator lists — otherwise ~240 correct matches would look like conflicts. A match is rejected only when both sides name a creator and they genuinely disagree (~180). Verified by sampling links back against `api/v1/games/{id}`.
...
Six credential-free read APIs (also usable by bots). All of them fetch the deployment's own `data/search_index.json` and return the **same enriched per-game record**: `id`, `title`, `creator`, `url`, `tags`, `engine`, `release_date`, `rating`, `difficulty`, `rating_count`, `file_size`, `page_url` (the `?game=<id>` deep link) and `source` (a **list** of `{type, id, site, url}` — every upstream Delicious Fruit / IWanna Wiki page for the game, ingest origin first; `[]` for submissions; §2.1). `rating`/`difficulty` are `null` for unrated games.
...
7c. **Source provenance stamp** — every mapped game gets its ingest origin written into `source` (§2.1) from the in-memory `seq_map`, after all claims/ingests and before the delta. Doing it here (rather than leaving it to `backfill_source_links.py`, §8.3) is what keeps the field on the *incremental* path: a backfill that assigned on every run would bump the version with no timeline entry every cycle and full-reload every client.

7d. **Wiki cross-link pass** — the same rules as `backfill_wiki_links.py`, applied to games the committed artifact never saw (i.e. ingested since it was generated), so the historical ~12k ride the artifact and the delta stays small. `fetch_wiki_games()` *breaks out of its page loop on error*, so a partial catalog is possible; the pass is therefore skipped unless the fetched catalog is at least as large as the `wiki_catalog_size` recorded in the artifact — matching on a thinned catalog would mislink, and skipping simply retries next run.
...
- `backfill_source_links.py` — permanent idempotent CI step (same slot): projects `database/seq_to_orig_map.json` onto each game as the first `source` entry (§2.1). Same no-bump-on-zero / bump-without-timeline contract. **Why it is a no-op in steady state:** the scraper stamps `source` inline before its delta generation (§8.1), so newly ingested games carry the field in the *normal* incremental update; if this script had to assign on every run, every run would force a full client reload. It remains in CI to catch remappings (a title-match claim moving a catalog id to a different upstream id).
- `backfill_wiki_links.py` — the wiki cross-link half of `source` (§2.1), in two modes. `--refresh-artifact` (dev machine) fetches the whole wiki catalog, matches, and rewrites the committed `data/wiki_links.json`; **a partial fetch aborts** rather than writing a thinner mapping, because with entries missing a genuinely ambiguous title looks unique and would be mislinked. The CI mode applies that artifact **offline** — no wiki call in CI, so a wiki outage can never change the result — re-checking 1:1 on wiki ids before each assignment. Same no-bump-on-zero / bump-without-timeline contract, and likewise a no-op in steady state because the scraper matches new games inline.
- `source_links.py` — not a script: the shared module holding the `source` list shape, `merge_source` and the wiki matching rules, imported by both backfills and the scraper so the rules exist in exactly one place.
